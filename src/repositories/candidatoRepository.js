...
class CandidatoRepository {
  ...
  async atualizarCurriculo(usuarioId, urlCurriculo, nomeCompleto, sobre, telefone) {
    const query = `
      ...
      RETURNING *
    `;
    const { rows } = await db.query(query, [urlCurriculo, nomeCompleto || null, sobre || null, telefone || null, usuarioId]);
    return rows[0];
  }
  ...
  async atualizarInfos(usuarioId, nomeCompleto, sobre, telefone, formacao, experiencias, habilidades) {
    const query = `
      ...
      RETURNING *
    `;
    const { rows } = await db.query(query, [nomeCompleto||null, sobre||null, telefone||null, formacao||null, experiencias||null, habilidades||null, usuarioId]);
    return rows[0];
  }

  async removerCurriculo(usuarioId) {
    const { rows } = await db.query(
      'UPDATE candidatos SET curriculo_url = NULL WHERE usuario_id = $1 RETURNING *',
      [usuarioId]
    );
    return rows[0];
  }
  async removerLaudo(usuarioId) {
    const { rows } = await db.query(
      'UPDATE candidatos SET laudo_medico_url = NULL WHERE usuario_id = $1 RETURNING *',
      [usuarioId]
    );
    return rows[0];
  }
  async atualizarFoto(usuarioId, fotoUrl) {
    const { rows } = await db.query(
      'UPDATE candidatos SET foto_url = $1 WHERE usuario_id = $2 RETURNING *',
      [fotoUrl, usuarioId]
    );
    return rows[0];
  }
  ...
  async removerFoto(usuarioId) {
    const { rows } = await db.query(
      'UPDATE candidatos SET foto_url = NULL WHERE usuario_id = $1 RETURNING *',
      [usuarioId]
    );
    return rows[0];
  }
